...
def load_data(input_dirs, nb_dim, pad=3):
    ...
    #print("type X[0]:", type(X[0]))
    X = X[:, None, ...]
    y = np_utils.to_categorical(np.asarray(y, dtype=int))
...
def build_model(nb_dim, nb_output):
    ...
    if 1:
        ...
        model.add(Conv2D(32, (5, 5), activation='relu', padding='valid', input_shape=(1, nb_dim, nb_dim),data_format="channels_first"))
        model.add(Conv2D(32, (5, 5), activation='relu', padding='valid',data_format="channels_first"))
...
if __name__ == '__main__':
    ...
    steps_per_epoch = int(n_train / args.batch_size)
